...
export default function DeepSeekChat() {
  // 【核心修复】
  // 1. 解构所有必要的变量 (input, handleInputChange, handleSubmit, isLoading)
  // 2. 移除 'api' 属性 (v4 已废弃)
  // 3. 使用自定义 'fetch' 来指定后端地址 '/api/deepseek/chat'
  const { 
    messages, 
    input, 
    handleInputChange, 
    ...
    id: 'deepseek-chat', // 可选：给会话一个唯一 ID
    async fetch(input, init) {
      // 强制请求你的特定 API 路径
      const response = await fetch('/api/deepseek/chat', init);
