...
export const testRoutes = app
    .get('/chat', describeRoute({
        ...
        try {
            ...
            return c.json(createErrorResult('测试接口调用失败', error.message), 500);
        }
    })
    .get('/health', describeRoute({
        ...
        try {
            ...
            return c.json(createErrorResult('健康检查失败', error.message), 500);
        }
    })
    .get('/echo', describeRoute({
        tags: testTags,
        summary: '回显测试接口',
